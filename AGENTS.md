...
10. `logrusbox/fleet` when the task has cross-component implications
...
- Durable consequential CIC Station decisions belong in CIC Station ADRs. Fleet-level decisions that genuinely span CIC Station and Vincent belong in `logrusbox/fleet`.
...
`logrusbox/cic-station` owns the reusable CIC Station web application, API, database schema/migrations, tests, packaging, component documentation, component-specific issues, and component roadmap. Generic Vincent worker implementation belongs in `logrusbox/vincent`. Fleet-wide roadmap, cross-component integration issues, and Fleet governance belong in `logrusbox/fleet`.
...
- `docs/ROADMAP.md` owns CIC Station component/release outcomes only.
- `docs/STATUS.md` records current CIC Station implementation/test state.
- `docs/PROGRAM_ROADMAP.md` is a migration pointer; the canonical Fleet roadmap lives in `logrusbox/fleet`.
- CIC Station-specific GitHub issues are the local backlog; cross-component issues belong in `logrusbox/fleet`.
