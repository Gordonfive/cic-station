...
Check current remote state in both `Gordonfive/mission-control` and `Gordonfive/vincent` before assuming a branch is current. Git is the durable authority; chat history is not.
...
- Mission Control must not become a general-purpose remote shell.
...
- `PROGRAM_ROADMAP.md` owns cross-product Vincent + Mission Control milestones.
- `ROADMAP.md` owns Mission Control product/release outcomes only.
