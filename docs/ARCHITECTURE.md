...
Mission Control is the managed-fleet control plane. Vincent is the worker platform. Project repositories remain authoritative for project source, requirements, tests, repository-local instructions, and durable development artifacts.

A fresh Vincent worker does not require Mission Control. Mission Control authority begins after explicit enrollment.
...
| Mission Control product requirements/ADRs/program roadmap | Mission Control Git repository |
| Worker identity/enrollment/authorization state | Mission Control service/database once implemented |
| Worker heartbeat and current liveness | Mission Control operational state |
| Assignment lease ownership | Mission Control operational state |
| Durable task result/source changes | Project Git plus Mission Control audit/reference |
| Installer provenance and current Vincent version | Reported by Vincent; recorded by Mission Control |
...
Mission Control exposes bounded operations and state transitions, not arbitrary shell execution. SSH remains an independent administrative fallback.
...
4. Mission Control grants scoped, revocable authority.
...
Assignments describe bounded work and hard requirements. Mission Control may explicitly select a worker or choose one by capabilities.
...
Mission Control may assign the desired provider and non-secret identity context. Vincent owns provider installation, provider-specific enrollment, local credential storage, and effective-identity/health verification.

Mission Control receives only non-secret effective identity/scope/status information needed to detect mismatches and enforce policy.
